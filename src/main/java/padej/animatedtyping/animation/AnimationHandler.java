...
import padej.animatedtyping.animation.types.*;
import padej.animatedtyping.config.ConfigManager;
import padej.animatedtyping.util.RemovedChar;
...
public class AnimationHandler {
    ...
    public void updateCharacters(String visibleText, String lastVisibleText,
        ...
        if (visibleText.equals(lastVisibleText)) return;

        long now = System.currentTimeMillis();

        // Find common prefix length
        int prefixLen = 0;
        int minLen = Math.min(visibleText.length(), lastVisibleText.length());
        while (prefixLen < minLen && visibleText.charAt(prefixLen) == lastVisibleText.charAt(prefixLen)) {
            prefixLen++;
        }
        ...
        while (suffixLen < minLen - prefixLen
                && visibleText.charAt(visibleText.length() - 1 - suffixLen) == lastVisibleText.charAt(lastVisibleText.length() - 1 - suffixLen)) {
            suffixLen++;
        }

        int oldChangedEnd = lastVisibleText.length() - suffixLen;
        int newChangedEnd = visibleText.length() - suffixLen;

        // Removed characters: old[prefixLen..oldChangedEnd)
        for (int i = prefixLen; i < oldChangedEnd; i++) {
            int widthUpTo = textRenderer.getWidth(lastVisibleText.substring(0, i));
            removedChars.put(i, new RemovedChar(lastVisibleText.charAt(i), now, textX + widthUpTo));
        }

        // Shift existing timestamps for characters after the change point
        int delta = visibleText.length() - lastVisibleText.length();
        if (delta != 0 && suffixLen > 0) {
            Map<Integer, Long> shifted = new java.util.HashMap<>();
            for (var entry : charTimestamps.entrySet()) {
                int idx = entry.getKey();
                if (idx >= prefixLen) {
                    shifted.put(idx + delta, entry.getValue());
                } else {
                    shifted.put(idx, entry.getValue());
                }
            }
            charTimestamps.clear();
            charTimestamps.putAll(shifted);
        }

        // New characters: new[prefixLen..newChangedEnd)
        for (int i = prefixLen; i < newChangedEnd; i++) {
